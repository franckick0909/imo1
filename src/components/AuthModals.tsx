...
export default function AuthModals({
          ...
          onError: (ctx) => {
            ...
            if (ctx.response) {
              switch (ctx.response.status) {
                ...
                case 404:
                  errorMessage =
                    "Aucun compte trouvé avec cet email. Vous êtes peut-être inscrit via Google ou GitHub ?";
                  break;
                case 403:
                  errorMessage =
          ...
          onError: (ctx) => {
            ...
            if (ctx.response) {
              switch (ctx.response.status) {
                case 409:
                  errorMessage =
                    "Un compte existe déjà avec cet email. Si vous êtes inscrit via Google/GitHub, connectez-vous d'abord avec cette méthode, puis ajoutez un mot de passe dans vos paramètres.";
                  break;
                case 400:
                  errorMessage = "Données invalides. Vérifiez vos informations";
                  break;
  ...
  const ModalHeader = ({ title }: { title: string }) => (
    ...
    >
      <motion.h2
        className="text-xl font-light text-zinc-900 uppercase"
